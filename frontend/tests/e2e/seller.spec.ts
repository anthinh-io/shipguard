...
function bodyFor(deliveredOrders: number) {
  return {
    ...
    // Backend quyết định cờ này, frontend chỉ vẽ lại — cùng phân công với độ mịn của
    // biểu đồ xu hướng ở #9.
    small_sample: deliveredOrders < 30,
  };
}
...
test("tập dưới 30 đơn hiện nhãn cảnh báo mà vẫn giữ nguyên số liệu", async ({ page }) => {
  await page.route(DASHBOARD_API, (route) => route.fulfill({ json: bodyFor(17) }));
  await mockSellers(page);
...
test("tập từ 30 đơn trở lên không hiện nhãn cảnh báo", async ({ page }) => {
  // Ranh giới nằm ở backend và được kiểm ở đó; đây chỉ khẳng định frontend vẽ đúng cờ
  // nhận được, không tự suy ra ngưỡng của riêng nó.
  await page.route(DASHBOARD_API, (route) => route.fulfill({ json: bodyFor(30) }));
  await mockSellers(page);
